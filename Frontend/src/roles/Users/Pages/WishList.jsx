...
export default function WishList() {
  ...
  const getFinalPrice = (product) => {
    ...
    // discountPrice هنا هو قيمة الخصم مش السعر النهائي — نفس منطق ProductCard
    return discountPrice > 0 ? price - discountPrice : price;
  };

  const hasDiscount = (product) => (product.discountPrice || 0) > 0;
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-text-subtle text-sm mt-1">
                ...
                {wishlistProducts.length <= 10 && wishlistProducts.length >= 3 ? 'منتجات' : 'منتج'}{' '}
                محفوظ
              </p>
            </div>
          </div>
        ...
        {!isLoading && wishlistProducts.length > 0 && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {wishlistProducts.map((product) => {
              ...
              const finalPrice   = getFinalPrice(product);   // price - discountPrice
              ...
              return (
                ...
                >
                  ...
                  <div className="flex flex-col grow">
                    <h3
                      className="font-bold text-text-main text-base mb-1 line-clamp-1"
                      title={productName}
                    >
                      {productName}
                    </h3>
                    ...
                    <button
                      onClick={(e) => {
                        e.preventDefault();
                        e.stopPropagation();
                        if (isInStock) handleAddToCart(productId);
                      }}
                      disabled={!isInStock}
