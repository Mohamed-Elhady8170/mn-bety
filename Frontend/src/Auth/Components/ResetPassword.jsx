...
import { resetPasswordThunk } from '../Features/authThunks';
...
const ResetPassword = () => {
    ...
    const getPasswordErrorMessage = (val) => {
        if (!val) return t('auth.validation.password_required');
        if (val.length < 8) return t('auth.validation.password_min');
        if (!/[a-z]/.test(val)) return t('auth.validation.password_lowercase');
        if (!/[A-Z]/.test(val)) return t('auth.validation.password_uppercase');
        if (!/\d/.test(val)) return t('auth.validation.password_number');
        return '';
    };
    ...
    const handleBlur = (field) => {
        ...
        if (field === 'password') {
            const err = getPasswordErrorMessage(password);
    ...
    const handleSubmit = async (e) => {
        ...
        if (!token) {
            setErrors(prev => ({ ...prev, password: 'رابط إعادة التعيين غير صالح أو منتهي الصلاحية' }));
            return;
        }
        ...
        <main className="flex-1 flex items-center justify-center p-4 md:p-6 bg-bg-light font-cairo min-h-[calc(100vh-80px)] relative overflow-hidden">
            <div className="absolute inset-0 overflow-hidden pointer-events-none">
                ...
                {floatingIcons.map((item, index) => {
                    ...
                    return (
                        <div key={index} className="absolute text-primary/30"
                            style={{ left: `${item.left}%`, top: `${item.top}%`, animation: `floatBg ${item.duration}s ease-in-out ${item.delay}s infinite` }}>
                            <Icon size={item.size} />
                        </div>
                    );
                })}
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div className="space-y-2 group">
                            ...
                            <div className="relative">
                                <Lock className={`absolute right-3 top-1/2 -translate-y-1/2 w-5 h-5 transition-all duration-300 ${touched.password && errors.password ? 'text-red-text' : 'text-text-subtle opacity-50 group-focus-within:opacity-100 group-focus-within:text-primary'}`} />
                                <input
                                    className={`w-full rounded-xl border ${touched.password && errors.password ? 'border-red-text' : 'border-border-warm'} bg-bg-subtle py-3 pr-10 pl-10 text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all duration-300 outline-none text-text-main placeholder:text-text-subtle`}
                            ...
                            {touched.password && errors.password && (
                                <p className="text-red-text text-xs pr-1 mt-1 bg-red-soft p-2 rounded-lg border border-red-200">{errors.password}</p>
                            )}
                        </div>
                        ...
                        <div className="space-y-2 group">
                            ...
                            <div className="relative">
                                <Lock className={`absolute right-3 top-1/2 -translate-y-1/2 w-5 h-5 transition-all duration-300 ${touched.confirmPassword && errors.confirmPassword ? 'text-red-text' : 'text-text-subtle opacity-50 group-focus-within:opacity-100 group-focus-within:text-primary'}`} />
                                <input
                                    className={`w-full rounded-xl border ${touched.confirmPassword && errors.confirmPassword ? 'border-red-text' : 'border-border-warm'} bg-bg-subtle py-3 pr-10 pl-10 text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all duration-300 outline-none text-text-main placeholder:text-text-subtle`}
                            ...
                            {touched.confirmPassword && errors.confirmPassword && (
                                <p className="text-red-text text-xs pr-1 mt-1 bg-red-soft p-2 rounded-lg border border-red-200">{errors.confirmPassword}</p>
                            )}
                        </div>
                        ...
                        {touched.password && (
                            <div className="text-xs text-text-subtle pr-1 space-y-1 bg-bg-subtle/50 p-3 rounded-lg border border-border-warm">
                                ...
                                <ul className="list-disc list-inside mr-2 space-y-1">
                                    <li className={password.length >= 8 ? 'text-green-600 dark:text-green-400' : ''}>• {t('auth.password_requirements.min_chars')}</li>
                                    <li className={/[a-z]/.test(password) ? 'text-green-600 dark:text-green-400' : ''}>• {t('auth.password_requirements.lowercase')}</li>
                                    <li className={/[A-Z]/.test(password) ? 'text-green-600 dark:text-green-400' : ''}>• {t('auth.password_requirements.uppercase')}</li>
                                    <li className={/\d/.test(password) ? 'text-green-600 dark:text-green-400' : ''}>• {t('auth.password_requirements.number')}</li>
                                </ul>
                            </div>
                        )}
                        ...
                        <div className="pt-2">
                            <button type="submit" disabled={isLoading}
                                className="w-full h-12 bg-primary hover:bg-[#d43d0a] text-white rounded-xl text-base font-bold shadow-lg shadow-primary/20 hover:shadow-xl hover:shadow-primary/30 active:scale-[0.98] transition-all flex items-center justify-center gap-2 group disabled:opacity-70 disabled:cursor-not-allowed">
                                {isLoading ? (
